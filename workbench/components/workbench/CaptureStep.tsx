...
interface Props {
  hasAudio: boolean; analyzeBusy: boolean; audioResetKey: number;
...
export function CaptureStep(props: Props) {
  ...
  return <section className="screen practice-capture">
    ...
    <article className="panel capture-panel">
      <details className="context-details"><summary>{c.context} <span>{c.optional}</span></summary><label htmlFor="practice-intent">{c.contextQuestion}</label><textarea id="practice-intent" maxLength={600} value={props.intent.takeaway} placeholder={c.contextPlaceholder} onChange={(e) => props.onIntentChange({ takeaway: e.target.value })} /></details>
